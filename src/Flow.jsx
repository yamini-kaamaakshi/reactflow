...
import {Drawer, Input, Segmented, Spin} from "antd";
...
import PlacementIscreated from "./PlacementIscreated.jsx"
...
const WorkFlow = ({apiServer, apiKey}) => {
    ...
    const renderForm = (triggercode) => {
        console.log("triggercode",triggercode);

        if (!selectedAction || !PlacementIscreated[selectedAction.code]) {
            return <div>Select an action to see the form.</div>;
        }

        const ActionForm = PlacementIscreated[selectedAction.code];
        return (
            <ActionForm
                handleFormSubmit={handleFormSubmit}
                sendAs={sendAs}
                setSendAs={setSendAs}
                actionCode={selectedAction.code} // Pass actionCode
                formData={formData} // Pass fetched form data
            />
        );
    };
...
// Your existing code for fetching triggers
...
// Your other logic follows here
    const handleActionSelection = (action) => {
        // Immediately set the selected action when an action is clicked
        setSelectedAction(action);

        // Close the action drawer after selection
        setActionDrawerVisible(false);

        // Open the form drawer to show the form based on the selected action
        setFormDrawerVisible(true);

        // Optionally log the selected action
